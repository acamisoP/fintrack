...
// カメラ版kakeiboと同一オリジン(acamisop.github.io)のため設定済みならそのまま引き継がれる。
...
// ジャンル → 色(One UIブルーを軸にした落ち着いたパレット。絵文字は使わない)
const GENRE_COLOR = {
  '食費': '#0381fe', '外食費': '#3ea8ff', '嗜好品': '#8e7bff', 'タバコ': '#b18cff',
  '日用品': '#23c562', '医療・健康': '#2ed3b7', '車両費': '#ffb74d', '交通費': '#ffd54f',
  '旅費': '#ff8a65', 'サブスク(固定費)': '#f06292', '防衛費': '#90a4ae', '仕事(経費)': '#4dd0e1',
  'その他': '#7f8c9b', '給与': '#23c562', '副収入': '#66d98a', '未分類': '#565f6b',
};
const gColor = g => GENRE_COLOR[g] || GENRE_COLOR['その他'];

// ---------- ブランドアイコン(店舗ロゴ or 頭文字アバター) ----------
// ロゴはGoogleのfaviconサービス(カメラ版のブランド表示と同じ仕組み)。
// ドメインが引けない店は頭文字アバター(ジャンル色背景)。将来的に商品写真へ差し替え予定。
const BRAND_DOMAINS = [
  [/ローソン/, 'lawson.co.jp'], [/ファミリーマート|ファミマ/, 'family.co.jp'],
  [/セブン|7-?イレブン/i, 'sej.co.jp'], [/ミニストップ/, 'ministop.co.jp'],
  [/デイリーヤマザキ/, 'daily-yamazaki.jp'], [/セイコーマート/, 'seicomart.co.jp'],
  [/イトーヨーカドー/, 'itoyokado.co.jp'], [/イオン|マックスバリュ/, 'aeon.co.jp'],
  [/西友|SEIYU/i, 'seiyu.co.jp'], [/ライフ/, 'lifecorp.jp'], [/オーケー/, 'ok-corporation.jp'],
  [/業務スーパー/, 'gyomusuper.jp'], [/成城石井/, 'seijoishii.co.jp'],
  [/ウエルシア/, 'welcia-yakkyoku.co.jp'], [/マツモトキヨシ|マツキヨ/, 'matsukiyo.co.jp'],
  [/ツルハ/, 'tsuruha.co.jp'], [/サンドラッグ/, 'sundrug.co.jp'],
  [/ココカラファイン/, 'cocokarafine.co.jp'], [/クリエイト/, 'create-sd.co.jp'],
  [/アイセイ薬局/, 'aisei.co.jp'], [/スギ薬局/, 'sugi-net.jp'],
  [/ヨドバシ/, 'yodobashi.com'], [/ビックカメラ/, 'biccamera.com'],
  [/ヤマダ電機|ヤマダデンキ/, 'yamada-denki.jp'], [/ケーズデンキ/, 'ksdenki.com'],
  [/エディオン/, 'edion.com'], [/ジョーシン|上新/, 'joshin.co.jp'],
  [/ドン・?キホーテ|ドンキ/, 'donki.com'], [/ダイソー/, 'daiso-sangyo.co.jp'],
  [/セリア/, 'seria-group.com'], [/ニトリ/, 'nitori.co.jp'], [/無印良品/, 'muji.com'],
  [/カインズ/, 'cainz.co.jp'], [/コーナン/, 'hc-kohnan.com'],
  [/ユニクロ/, 'uniqlo.com'], [/GU/, 'gu-global.com'], [/しまむら/, 'shimamura.gr.jp'],
  [/マクドナルド|マック/, 'mcdonalds.co.jp'], [/モスバーガー/, 'mos.jp'],
  [/すき家/, 'sukiya.jp'], [/吉野家/, 'yoshinoya.com'], [/松屋/, 'matsuyafoods.co.jp'],
  [/サイゼリヤ/, 'saizeriya.co.jp'], [/ガスト|バーミヤン|ジョナサン/, 'skylark.co.jp'],
  [/CoCo壱|ココイチ|カレーハウス/i, 'ichibanya.co.jp'],
  [/くら寿司/, 'kurasushi.co.jp'], [/スシロー/, 'akindo-sushiro.co.jp'], [/はま寿司/, 'hama-sushi.co.jp'],
  [/スターバックス|スタバ/, 'starbucks.co.jp'], [/ドトール/, 'doutor.co.jp'],
  [/コメダ/, 'komeda.co.jp'], [/タリーズ/, 'tullys.co.jp'],
  [/モバイルSuica|MOBILE\s*SUICA|JR東日本/i, 'jreast.co.jp'],
...
const SVG_OFFLINE = '<svg viewBox="0 0 24 24" fill="none" stroke="#494950" stroke-width="1.8" stroke-linecap="round" style="width:44px;height:44px;margin-bottom:14px"><path d="M5 12.5a7 7 0 0 1 14 0M8 15.5a4 4 0 0 1 8 0"/><circle cx="12" cy="19" r="1.2" fill="#494950"/><path d="M3 3l18 18"/></svg>';
...
const SVG_EMPTY = '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 8.5 6 4h12l2 4.5V19a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1zM4 8.5h16M12 12v4M12 12h.01"/></svg>';
const SVG_SEARCH = '<svg viewBox="0 0 24 24" fill="none" stroke-width="1.8" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.2-3.2M8.5 11h5"/></svg>';
...
function renderHome() {
  ...
  html += '<div class="row-label">支払方法(今月)</div><div class="paycards" id="paycards"></div>' +
    '<div class="row-label">最近の取引<button id="seeall">すべて見る ›</button></div>' +
  ...
  const pc = $('paycards');
  if (d.byPay.length === 0) pc.innerHTML = '<div class="sub" style="padding:6px 8px">まだ支出がありません</div>';
  d.byPay.forEach(pv => {
    ...
    pc.appendChild(cardEl);
  });

  const recent = $('recent');
  d.tx.slice(0, 5).forEach(t => recent.appendChild(txRow(t, d, true)));
  if (!d.tx.length) recent.innerHTML = '<div class="empty">' + SVG_EMPTY + '<div>今月の取引はまだありません</div></div>';
...
function renderTxList() {
  ...
  const match = t => {
    ...
    const its = d.itemsByTx[t.i] || [];
    return its.some(it => it.n !== NA && it.n.toLowerCase().indexOf(q) !== -1);
  };
  const list = d.tx.filter(match);
  if (!list.length) {
    wrap.innerHTML = '<div class="empty">' + (q ? SVG_SEARCH : SVG_EMPTY) + '<div>' +
      (q ? '「' + esc(state.q) + '」に一致する取引はありません' : 'この月の取引はありません') + '</div></div>';
    return;
  }
  const groups = [];
  list.forEach(t => {
...
function txRow(t, d, compact) {
  ...
  const sub = compact
    ? (dt.getMonth() + 1) + '/' + dt.getDate() + (t.p ? '・' + t.p : '')
    : (t.p || t.st || t.s) + (t.st && t.p ? '・' + t.st : '');
  row.appendChild(iconNode(t.n, gColor(g)));
  const mid = document.createElement('div'); mid.className = 'mid';
  const nm = document.createElement('div'); nm.className = 'nm'; nm.textContent = t.n;
  const dtEl = document.createElement('div'); dtEl.className = 'dt'; dtEl.textContent = sub;
  mid.appendChild(nm); mid.appendChild(dtEl);
  const amt = document.createElement('div'); amt.className = 'amt' + (inc ? ' inc' : '');
  amt.textContent = (inc ? '+' : '') + yen(t.a);
  row.appendChild(mid); row.appendChild(amt);
...
function buildDonut(svg, segs, total) {
  ...
  svg.innerHTML = '<circle cx="100" cy="100" r="74" fill="none" stroke="var(--card2)" stroke-width="26"/>';
  if (!total) return;
  let acc = 0;
  segs.forEach((s, i) => {
    ...
    el.setAttribute('stroke', gColor(s[0]));
...
function buildLegend(wrap, segs, total) {
  ...
  wrap.innerHTML = segs.map((s, i) =>
    ...
    '<span class="dot" style="background:' + gColor(s[0]) + '"></span>' +
    '<span class="g">' + esc(s[0]) + '</span>' +
    '<span class="p">' + (total ? Math.round(s[1] / total * 100) : 0) + '%</span>' +
    '<span class="v">' + yen(s[1]) + '</span></div>' +
    '<div class="bar"><i style="background:' + gColor(s[0]) + ';width:' +
...
function openSheet(t, d) {
  ...
  const inc = t.k === '収入';
  const dt = new Date(t.d + 'T00:00:00');
  // 明細1件で「取引名のコピー」または「N/A(商品名不明)」の場合は明細表を出さない
  const single = its.length === 1 && (its[0].n === t.n || its[0].n === NA || !its[0].n);
  let html = '<div class="sh-head"><span id="sh-ic"></span><div class="sh-title">' + esc(t.n) + '</div></div>' +
    '<div class="sh-amt' + (inc ? ' inc' : '') + '">' + (inc ? '+' : '') + yen(t.a) + '</div>' +
    '<div class="chips">' +
    '<span class="chip">' + dt.getFullYear() + '/' + (dt.getMonth() + 1) + '/' + dt.getDate() +
    '(' + WD[dt.getDay()] + ')</span>' +
    (t.p ? '<span class="chip">' + esc(t.p) + '</span>' : '') +
    (t.st ? '<span class="chip">' + esc(t.st) + '</span>' : '') +
    (t.s ? '<span class="chip">' + esc(t.s) + '</span>' : '') +
    '<span class="chip g" style="background:' + gColor(g) + '55">' + esc(g) + '</span>' +
    '</div>';
  if (its.length && !single) {
    html += '<div class="sh-items">' + its.map(it =>
      '<div class="sh-item"><span class="nm">' + esc(it.n === NA ? '(商品名なし)' : it.n) + '</span>' +
  ...
  $('sh-ic').replaceWith(iconNode(t.n, gColor(g)));
  $('sh-notion').onclick = () => {
    vib(8);
